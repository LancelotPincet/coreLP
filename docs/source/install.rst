...
Then, we just need to add the dependencies to your project, typically dependency to **coreLP**, with the command:

.. code-block:: shell

   uv add corelp

You can redo this command for all the dependencies you might need by replacing the "coreLP" by the other dependency name (the name ususally used in the command *pip*).
...
.. tabs::
   ...
   .. tab:: Anaconda tools (Spyder Jupyter)
      ...
      To be able to use another virtual environment, *Spyder* and *Jupyter* connects to specific kernels, respectively *spyder-kernels* and *ipykernel*, which are included in **coreLP** library dependencies.
...
To finish, some scripts may be provided with the library **coreLP** :doc:`(see Reference Guide) <scripts>`.
To use them, you can just download source codes from the `GitHub repository <https://github.com/LancelotPincet/coreLP/tree/main/src/corelp/scripts>`_.
...
.. dropdown:: Using *Anaconda* distribution
   ...
   *Anaconda* is a Python distribution designed for scientific programming.
   ...
   Here we will explain how to setup up virtual environments dependent on **coreLP** that you will be able to use on various projects.
   ...
   However some dependencies for this Python project might not be available in this *base* environment, like **coreLP**.
   ...
   Use these commands in the *Anaconda prompt* to set up a virtual environment :
      - create venv: ``conda create -n venv_coreLP python``
      - activate venv: ``conda activate venv_coreLP``
      - update pip: ``conda install pip``
      - add *Spyder* and *Jupyter* kernels: ``conda install spyder-kernels ipykernel``
      - install **coreLP**: ``pip install corelp``
...
.. dropdown:: Manual project
   ...
   If you do not want to use any external tool and have lots of patience, you can do each step manually.
      ...
      6. Install **coreLP** from terminal: ``pip install corelp``
...
If ever you want to play with the library source code, to apply your own modifications, or even to contribute, you can access it on the `GitHub page <https://github.com/LancelotPincet/coreLP>`_.
...
Now, you can clone in your current directory the `GitHub repository <https://github.com/LancelotPincet/coreLP>`_ with the following command
.. code-block:: shell
