...
Modules go in `src/corelp/modules/`.
Routines go in `src/corelp/routines/`.
...
- Public objects must be exposed in `corelp/__init__.py`
...
```python
# RIGHT
from corelp import MyObject

# WRONG — never import from internal paths
from corelp.modules.myobject_LP.myobject import MyObject
```

Every public object must be registered in `corelp/__init__.py`:

```python
sources = {
    "<name>": "corelp.modules.<name>_LP.<name>",
}
```
